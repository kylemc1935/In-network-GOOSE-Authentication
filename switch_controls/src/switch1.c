...
const char *csv_path = "../data/algorithm_data/algorithm_experiment_timing.csv"; // for outputting isolated crypto timings
...
// computes the time between two timestamps
static inline uint64_t dt_ns(uint64_t t0, uint64_t t1) { return t1 - t0; }

// derives nonce from increasing counter
...
// authentication handler for S1 (authentication switch) in MAC mode
// computes tag over the packet and inserts the tag into the extension field and forrwards on
...
//    uint64_t t_crypto0 = now_ns();
    ...
    uint8_t tag[64];
    size_t tag_len = 0;
    // authenticate everything except the trailer
...
// verification handler for S2 (verification switch) in MAC mode
// parses authentiation extension and verifies against generated tag, then forwards is successful
int handle_verify(ctx_t *ctx, const struct pcap_pkthdr *header, const u_char *packet){
    ...
    &nonce, &tag, &original_len)){
        printf("*** tag size unexpected, unable to verify ***\n");
        return 0;
    }

    // similarly copy the not authenticate the trailer
    size_t protected_len = original_len - ID_TRAILER_LEN;

//    uint64_t t_crypto0 = now_ns();

    // verify tag aginst transmitted tag and forward without the extension field
...
// aead handler for S1
// authenticates the ethernet header as aad, encrypts and authenticates the payload
// inserts the nonce and tag to the extension field and forwards appropiately
int handle_aead_encrypt(ctx_t *ctx, const struct pcap_pkthdr *header, const u_char *packet)
{
    ...
    // everything after the ethernet header (and before the id trailer) is plaintext (encrypted)
...
// aead handler for S2
// parses the nonce and tag from extension and verifies the ciphertext and tag
// if successful, forwarrds the decrypted packet, without the extension
...
//    uint64_t t_crypto0 = now_ns();
    ...
    if (!aead_decrypt_verify_inplace(p, &ctx->crypto, aad, aad_len, buf + ct_off, ct_len, nonce, p->nonce_len, tag, p->tag_len)){
        printf("******* AEAD verify/decrypt failed *******\n");
        return 0;
    }
//    uint64_t t_crypto1 = now_ns();
//    ctx->crypto_last_ns = dt_ns(t_crypto0, t_crypto1);
...
// libpcap function call for all captured packets
// filtes for GOOSE and executes the appropiate function given the profile
...
//    uint64_t t_total0 = now_ns();
    ...
    // timing measurments for recoring time and debugging (ignore)
...
// chooses the packet handler based on the switch role and profile mode
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 3) {
        fprintf(stderr, "Usage: %s <sw:1|2|3> <profile_id:1-6>\n", argv[0]);
        return 1;
    }

    // selects the logical switch role and corresponding capture/send interfaces
    // REMEMBER : the actual experiment listens on the veth/tap interfaces - NOT the physical ports
    if (strcmp(argv[1], "1") == 0) { // 1 = mininet S1
        ctx.sw_id = SW_S1;
        capture_iface = "S1-eth1";
        send_iface    = "S1-eth2";
    } else if (strcmp(argv[1], "2") == 0) { // 2 = mininet S2
        ctx.sw_id = SW_S2;
        capture_iface = "S2-eth1";
        send_iface    = "S2-eth2";
    } else if (strcmp(argv[1], "3") == 0) { // 3 = mininet S1
        ctx.sw_id = SW_S1;
        capture_iface = "veth2";
        send_iface    = "veth4";
    } else if (strcmp(argv[1], "4") == 0) { // 4 = mininet S2
        ctx.sw_id = SW_S2;
        capture_iface = "veth2";
        send_iface    = "veth4";
    } else if (strcmp(argv[1], "5") == 0) { // 4 = mininet S2
        ctx.sw_id = SW_S1;
        capture_iface = "vtap0";
        send_iface    = "vtap0";
    } else if (strcmp(argv[1], "6") == 0) { // 4 = mininet S2
        ctx.sw_id = SW_S2;
        capture_iface = "vtap0";
        send_iface    = "vtap0";
    } else {
    ...
    // argv[2] = profile id as a single digit from cl
...
//    }
    ...
    // choose the packet action/alg based on switch + mode
